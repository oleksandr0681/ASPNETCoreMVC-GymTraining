...
using GymTraining.Models.AccountViewModels;
...
public class AccountController : Controller
{
    ...
    private readonly string userRoleSportsman = "Sportsman";
    private readonly string userRoleTrainer = "Trainer";
    ...
    public async Task<IActionResult> Register(RegisterViewModel model, string returnUrl = null)
    {
        ...
        if (ModelState.IsValid)
        {
            if (model.Role != userRoleSportsman && model.Role != userRoleTrainer)
            {
                ModelState.AddModelError("Role", "Помилка в ролі користувача.");
                // If we got this far, something failed, redisplay form
                return View(model);
            }
            ...
            if (result.Succeeded)
            {
                ...
                if (model.Role == userRoleSportsman)
                {
                    await _userManager.AddToRoleAsync(user, userRoleSportsman);
                    TrainerData? trainerData = await _context.TrainersData.FirstOrDefaultAsync();
                    if (trainerData != null)
                    {
                        ...
                        { 
                            ...
                            Name = model.FullName, 
                            TrainerDataId = trainerData.Id,
                        };
                        _context.Add(sportsmanData);
                        await _context.SaveChangesAsync();
                    }
                }
                else if (model.Role == userRoleTrainer)
                {
                    await _userManager.AddToRoleAsync(user, userRoleTrainer);
    ...
    public async Task<IActionResult> DeleteConfirmed(string Id)
    {
        ...
        if (userRoles.Contains("Trainer"))
        {
            TrainerData? trainerData = _context.TrainersData
                .Where(t => t.ApplicationUserId == user.Id).FirstOrDefault();
            if (trainerData != null)
            {
                ViewData["ErrorMessage"] = "Не можна вилучити обліковий запис " +
                        "тренера, поки є інформація про тренера.";
                return View("HandleError");
            }
        }
        if (userRoles.Contains("Sportsman"))
        {
            SportsmanData? sportsmanData = _context.SportsmenData
                .Where(s => s.ApplicationUserId == Id).FirstOrDefault();
            if (sportsmanData != null)
            {
                ViewData["ErrorMessage"] = "Не можна вилучити обліковий запис " +
                    "спортсмена, поки є інформація про спортсмена.";
                return View("HandleError");
            }
        }
